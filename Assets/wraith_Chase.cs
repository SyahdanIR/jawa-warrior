...
public class wraith_Chase : StateMachineBehaviour
{
    Rigidbody2D rb;
    ...
    {
        wraith.Instance.Flip();
        Vector2 _target = new Vector2(Playercontroller.Instance.transform.position.x, rb.position.y);
        Vector2 _newPos = Vector2.MoveTowards(rb.position, _target, wraith.Instance.runSpeed * Time.fixedDeltaTime);
        wraith.Instance.runSpeed = wraith.Instance.speed;
